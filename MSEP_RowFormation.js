 ...
 * @parent ---Row 3 Settings---
 * @desc The help description used for this row.
 * @default Place allies here to take less melee daMageStudios.
 *
 * @param Row 3 States
 * @parent ---Row 3 Settings---
 * @desc When a battler is in this row, apply these states.
 ...
 *
 *       Name: MageStudios.Param.RowCmdName
 *     Symbol: row
 ...
 *   This requires the MSEP_BattleEngineCore plugin to take effect.
 *
 *   EnableBattleRow
 *   DisableBattleRow
 *   This will either enable or disable the Row command from the battle party
 *   menu. This requires the MSEP_BattleEngineCore plugin to take effect.
 *
...
MageStudios.SetupParameters = function () {
  for (var i = 1; i < 11; ++i) {
    ...
    if (Utils.RPGMAKER_VERSION && Utils.RPGMAKER_VERSION >= "1.5.0") {
      ...
      MageStudios.Row.States[i] = [];
      for (var j = 0; j < array.length; ++j) {
        if (MageStudios.Row.States[i].contains(array[j])) continue;
        MageStudios.Row.States[i].push(parseInt(array[j]));
      }
    }
    text = "Row " + i + " Home X";
...
DataManager.isDatabaseLoaded = function () {
  ...
  if (!MageStudios._loaded_MSEP_RowFormation) {
    ...
    MageStudios._loaded_MSEP_RowFormation = true;
  }
  return true;
};
...
DataManager.processRowNotetags1 = function (group) {
  for (var n = 1; n < group.length; n++) {
    ...
    obj.defaultRow = [MageStudios.Param.RowDefault];
...
if (Imported.MSEP_BattleEngineCore) {
  ...
  BattleManager.playBattleBgm = function () {
    ...
    if (MageStudios.Row.SavedBattleBgm) {
      ...
      MageStudios.Row.SavedBattleBgm = undefined;
      restartBgm = false;
    }
    if (MageStudios.Row.SavedBattleBgs) {
      AudioManager.playBgs(MageStudios.Row.SavedBattleBgs);
      MageStudios.Row.SavedBattleBgs = undefined;
      restartBgm = false;
    }
  ...
  Scene_Battle.prototype.partyCommandRow = function () {
    ...
    MageStudios.Row.SavedBattleBgm = AudioManager.saveBgm();
