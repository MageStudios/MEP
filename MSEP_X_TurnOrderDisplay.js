...
if (Imported.MSEP_BattleEngineCore) {
  if (MageStudios.BEC.version && MageStudios.BEC.version >= 1.42) {
    MageStudios.Parameters = PluginManager.parameters(
      "MSEP_X_TurnOrderDisplay"
    );
    MageStudios.Param = MageStudios.Param || {};

    MageStudios.Param.TODShowDefault = String(
      MageStudios.Parameters["Show Turn Order"]
    );
    ...
    MageStudios.Param.TODPositionX = String(
      MageStudios.Parameters["Position X"]
    );
    MageStudios.Param.TODPositionY = Number(
      MageStudios.Parameters["Position Y"]
    );
    MageStudios.Param.TODTurnDirection = String(
      MageStudios.Parameters["Turn Direction"]
    );
    MageStudios.Param.TODPerformedSpace = String(
      MageStudios.Parameters["Performed Spacing"]
    );
    MageStudios.Param.TODPerformedSpace = eval(
      MageStudios.Param.TODPerformedSpace
    );

    MageStudios.Param.TODColorAllyBr = Number(
      MageStudios.Parameters["Ally Border Color"]
    );
    MageStudios.Param.TODColorAllyBg = Number(
      MageStudios.Parameters["Ally Back Color"]
    );
    MageStudios.Param.TODAllyIcon = Number(MageStudios.Parameters["Ally Icon"]);

    MageStudios.Param.TODColorEnemyBr = Number(
      MageStudios.Parameters["Enemy Border Color"]
    );
    MageStudios.Param.TODColorEnemyBg = Number(
      MageStudios.Parameters["Enemy Back Color"]
    );
    MageStudios.Param.TODEnemyIcon = Number(
      MageStudios.Parameters["Enemy Icon"]
    );
    MageStudios.Param.TODEnemySVBattler = String(
      MageStudios.Parameters["Enemy SV Battlers"]
    );
    MageStudios.Param.TODEnemySVBattler = eval(
      MageStudios.Param.TODEnemySVBattler
    );

    MageStudios.TOD.DataManager_isDatabaseLoaded = DataManager.isDatabaseLoaded;
    DataManager.isDatabaseLoaded = function () {
      if (!MageStudios.TOD.DataManager_isDatabaseLoaded.call(this))
        return false;
      ...
      for (var n = 1; n < group.length; n++) {
        ...
        for (var i = 0; i < notedata.length; i++) {
          ...
          if (line.match(/<(?:TURN ORDER ICON|TURN ICON):[ ](\d+)>/i)) {
            ...
            line.match(/<(?:TURN ORDER BACKGROUND COLOR):[ ](\d+)>/i)
          ) {
            obj.turnOrderBackgroundColor = parseInt(RegExp.$1);
          } else if (
            line.match(/<CLASS[ ](\d+)[ ]TURN ORDER ICON:[ ](\d+)>/i)
          ) {
    ...
    MageStudios.TOD.BattleManager_makeActionOrders =
    ...
    BattleManager.setTurnOrderDisplaySurprisePreemptiveDefault = function () {
      ...
      if (this._surprise) {
        var change = true;
        this._performedBattlers = this._performedBattlers.concat(
          $gameParty.members()
        );
    ...
    MageStudios.TOD.Game_Action_setItemObject =
      Game_Action.prototype.setItemObject;
    ...
    Game_Temp.prototype.battleTurnOrder = function () {
      ...
      if (
        MageStudios.Param.TODPerformedSpace &&
        this._battleTurnOrder.length > 0
      ) {
    ...
    Window_TurnOrderIcon.prototype.initSettings = function (
      unit,
      battlerIndex
    ) {
    ...
    Window_TurnOrderIcon.prototype.calculateDestinationXAlign = function () {
      ...
      if (this._position === "left") {
        ...
        if (this._direction === "left") {
          this._destinationX += Math.floor(
            (this.aliveMembersSize() * size) / 2
          );
    ...
    Window_TurnOrderIcon.prototype.getPerformedBattlers = function () {
      var result = [];
      var battlers = MageStudios.Util.getUnique(
        BattleManager._performedBattlers
      );
      var length = battlers.length;
      for (var i = 0; i < length; ++i) {
        var battler = battlers[i];
    ...
    var text =
      "================================================================\n";
    text +=
      "MSEP_X_TurnOrderDisplay requires MSEP_BattleEngineCore to be at the ";
    text +=
