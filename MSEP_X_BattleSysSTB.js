...
if (Imported.MSEP_BattleEngineCore) {
  if (MageStudios.BEC.version && MageStudios.BEC.version >= 1.42) {
    ...
    MageStudios.Param.STBActionSpeed = String(
      MageStudios.Parameters["Action Speed"]
    );
    MageStudios.Param.STBDelayStatus = String(
      MageStudios.Parameters["Delay Status Adjust"]
    );
    MageStudios.Param.STBDelayStatus = eval(MageStudios.Param.STBDelayStatus);

    MageStudios.Param.STBEscapeRatio = String(
      MageStudios.Parameters["Escape Ratio"]
    );
    MageStudios.Param.STBEscapeBoost = String(
      MageStudios.Parameters["Fail Escape Boost"]
    );

    MageStudios.Param.STBShowWindow = String(
      MageStudios.Parameters["Show Turn Window"]
    );
    MageStudios.Param.STBShowWindow = eval(MageStudios.Param.STBShowWindow);
    MageStudios.Param.STBWindowX = String(
      MageStudios.Parameters["Turn Window X"]
    );
    MageStudios.Param.STBWindowY = String(
      MageStudios.Parameters["Turn Window Y"]
    );
    MageStudios.Param.STBWindowW = String(
      MageStudios.Parameters["Turn Window Width"]
    );
    MageStudios.Param.STBWindowH = String(
      MageStudios.Parameters["Turn Window Height"]
    );
    MageStudios.Param.STBWinSubject = Number(
      MageStudios.Parameters["Current Battler Color"]
    );
    MageStudios.Param.STBWinActor = Number(
      MageStudios.Parameters["Actor Battler Color"]
    );
    MageStudios.Param.STBWinEnemy = Number(
      MageStudios.Parameters["Enemy Battler Color"]
    );

    MageStudios.STB.DataManager_isDatabaseLoaded = DataManager.isDatabaseLoaded;
    DataManager.isDatabaseLoaded = function () {
      if (!MageStudios.STB.DataManager_isDatabaseLoaded.call(this))
        return false;
    ...
    MageStudios.STB.BattleManager_makeEscapeRatio =
    ...
    MageStudios.STB.BattleManager_selectNextCommand =
      ...
      if (BattleManager.isSTB()) {
        ...
        try {
          ...
          MageStudios.Util.displayError(
            e,
            code,
            "STB ACTION SPEED FORMULA ERROR"
          );
        }
        return speed;
      } else {
    ...
    MageStudios.STB.Scene_Battle_commandFight =
    ...
    MageStudios.STB.Scene_Battle_commandEscape =
    ...
    Window_STB_TurnOrder.prototype.initialize = function (statusWindow) {
      ...
      try {
        ...
        MageStudios.Util.displayError(
          e,
          code,
          "STB TURN ORDER WINDOW WIDTH ERROR"
        );
        var width = 200;
      }
      ...
      try {
        ...
        MageStudios.Util.displayError(
          e,
          code,
          "STB TURN ORDER WINDOW HEIGHT ERROR"
        );
