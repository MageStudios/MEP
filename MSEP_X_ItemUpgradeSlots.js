...
if (Imported.MSEP_ItemCore) {
  ...
  ItemManager.initSlotUpgradeNotes = function (item) {
    ...
    item.upgradeSlots = MageStudios.Param.IUSDefaultSlots;
    item.upgradeSlotsVariance = MageStudios.Param.IUSSlotVariance;
    item.upgradeSound = MageStudios.Param.IUSUpgradeSound;
    item.slotsApplied = [];
  ...
  Window_ItemInfo.prototype.drawSlotUpgradesUsed = function (dy) {
    ...
    if (!DataManager.isIndependent(item)) return dy;
    if (baseItem.upgradeSlots <= 0) return dy;
    if (!eval(MageStudios.Param.IUSShowSlots)) return dy;
    if (item.slotsApplied.length <= 0) return dy;
    var dx = this.textPadding();
    var fmt = MageStudios.Param.IUSSlotFmt;
