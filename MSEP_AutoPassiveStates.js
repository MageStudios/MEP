...
MageStudios.SetupParameters = function () {
  ...
  MageStudios.Param.APSActorPas = String(
    MageStudios.Parameters["Actor Passives"]
  );
  MageStudios.Param.APSActorPas = MageStudios.Param.APSActorPas.split(" ");
  for (var i = 0; i < MageStudios.Param.APSActorPas.length; ++i) {
    MageStudios.Param.APSActorPas[i] = parseInt(
      MageStudios.Param.APSActorPas[i]
    );
  ...
  MageStudios.Param.APSEnemyPas = String(
    MageStudios.Parameters["Enemy Passives"]
  );
  MageStudios.Param.APSEnemyPas = MageStudios.Param.APSEnemyPas.split(" ");
  for (var i = 0; i < MageStudios.Param.APSEnemyPas.length; ++i) {
    MageStudios.Param.APSEnemyPas[i] = parseInt(
      MageStudios.Param.APSEnemyPas[i]
    );
  ...
  MageStudios.Param.APSGlobalPas = String(
    MageStudios.Parameters["Global Passives"]
  );
...
Game_BattlerBase.prototype.passiveStateConditions = function (state) {
  ...
  try {
    condition = state.passiveCondition.call(
      this,
      condition,
      a,
      user,
    ...
    MageStudios.Util.displayError(
      e,
      code,
      "PASSIVE STATE CUSTOM CONDITION ERROR"
    );
  }
...
Game_BattlerBase.prototype.passiveStateConditionEval = function (state) {
  ...
  try {
    condition = state.passiveConditionEval.call(
      this,
      condition,
      a,
      user,
...
MageStudios.APS.Game_Battler_isStateAddable =
