 ...
 * ============================================================================
 *
 * This plugin requires MSEP_EquipCore.js to work.
 * Place this plugin beneath MSEP_EquipCore.js in the Plugin Manager list.
 *
...
Window_EquipRequirement.prototype.drawRequirements = function(dy) {
    ...
    if (MageStudios.Param.EqReqSkills) dy = this.drawSkillRequirements(dy);
...
Window_EquipRequirement.prototype.drawClassRequirements = function(dy) {
    ...
    ww -= dx;
    if (style === 1 && Imported.MSEP_ClassChangeCore) {
      for (var i = 0; i < length; ++i) {
      ...
      dy += this.lineHeight();
    } else if (style === 2 && Imported.MSEP_ClassChangeCore) {
      for (var i = 0; i < length; ++i) {
        var classId = classes[i];
        this.changePaintOpacity(classId === currentClassId);
