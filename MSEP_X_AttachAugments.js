 ...
 * - Requires MSEP_AutoPassiveStates.js installed. Add/Remove passive state 'x'
 ...
 * picture iMageStudios. This requires the plugin: Item Picture Images.
 *
 * ---
 *
 * Change Prefix: x
...
ItemManager.applyAugmentAttackElement = function(mainItem, element, add) {
    if (element.match(/(\d+)/i)) {
      ...
      var id = MageStudios.ElementIdRef[element];
      if (!id) return;
    }
...
Window_ItemActionCommand.prototype.addAugmentSlots = function() {
    ...
    for (var i = 0; i < length; ++i) {
      ...
      var fmt = MageStudios.Param.AugmentSlotFmt;
