 ...
 * ============================================================================
 *
 * This plugin requires MSEP_ShopMenuCore.
 * Make sure this plugin is located under MSEP_ShopMenuCore in the plugin list.
 *
...
DataManager.adjustProxyBuy = function(obj, itemType) {
    ...
    if (MageStudios.Param.MCCopyName) obj.name = item.name;
    if (MageStudios.Param.MCCopyIcon) obj.iconIndex = item.iconIndex;
    if (MageStudios.Param.MCCopyHelp) obj.description = item.description;
...
Window_Gold.prototype.refresh = function() {
    ...
    if (this.isDrawGoldCurrency()) {
      MageStudios.MC.Window_Gold_refresh.call(this);
...
Window_ShopNumber.prototype.drawVariablePrice = function(ww, wy, varId) {
    ...
    if (this.isSelling()) {
      ...
      value = this._number * price;
      value = MageStudios.Util.toGroup(value * -1);
    }
    fw = Math.min(fw, this.drawAltCurrency(value, unit, 0, fy, ww));
    fy += this.lineHeight();
