...
ItemManager.makeDisassembleItems = function(line, type, results) {
    ...
    } else if (MageStudios.ItemIdRef[line]) {
      var id = MageStudios.ItemIdRef[line];
      var item = $dataItems[id];
    } else if (MageStudios.WeaponIdRef[line]) {
      var id = MageStudios.WeaponIdRef[line];
      var item = $dataWeapons[id];
    } else if (MageStudios.ArmorIdRef[line]) {
...
ItemManager.disassemble = function(targetItem, effectItem) {
    ...
    for (var i = 0; i < length; ++i) {
      ...
      rate += rates[index] || 0;
      if (Math.random() < rate) {
        var total = MageStudios.Util.getRandomInt(quantity1, quantity2);
...
Window_DisassemblePool.prototype.drawDisassembleTypePool = function(dy, type) {
    ...
    var length = pool.length;
    var fmt1 = MageStudios.Param.IDAQuantityFmt1;
    var fmt2 = MageStudios.Param.IDAQuantityFmt2;
    var dx = this.textPadding();
    this.resetFontSettings();
    var dw = this.contentsWidth() - dx * 2;
    for (var i = 0; i < length; ++i) {
