...
MageStudios.Param.JpShowResults = eval(
  String(MageStudios.Parameters["Show Results"])
);
MageStudios.Param.JpTextFormat = String(
  MageStudios.Parameters["JP Gained in Battle"]
);
MageStudios.Param.JpAliveActors = eval(
  String(MageStudios.Parameters["Alive Actors"])
);
...
MageStudios.Param.JpEnableAftermath = String(
  MageStudios.Parameters["Enable Aftermath"]
);
MageStudios.Param.JpAftermathText = String(
  MageStudios.Parameters["Aftermath Text"]
);
MageStudios.Param.JpAftermathFmt = String(
  MageStudios.Parameters["Aftermath Format"]
);
MageStudios.Param.JpAftermathEarn = String(
  MageStudios.Parameters["Aftermath JP Earned"]
);
...
BattleManager.displayJpGain = function () {
  ...
  $gameParty.members().forEach(function (actor) {
    ...
    var text = fmt.format(
      actor.name(),
      MageStudios.Util.toGroup(actor.battleJp()),
...
MageStudios.JP.Game_Battler_onBattleStart =
...
MageStudios.JP.Window_Base_drawActorClass =
...
if (Imported.MSEP_VictoryAftermath && MageStudios.Param.JpEnableAftermath) {
  ...
  Window_VictoryJp.prototype.drawJpGained = function (actor, rect) {
    ...
    this.drawText(
      MageStudios.Param.JpAftermathEarn,
      rect.x + 2,
      wy,
      rect.width - 4,
      "left"
    ...
    var bonusJp =
      (1.0 * actor.battleJp() * this._tick) / MageStudios.Param.VAGaugeTicks;
