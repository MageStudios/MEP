...
DataManager.processElementNotetags1 = function(group) {
  ...
  for (var n = 1; n < group.length; n++) {
    ...
    obj.elementMultiRule = MageStudios.Param.EleMultiRule;
...
BattleManager.actionAddElement = function(actionArgs) {
    ...
    for (var i = 0; i < length; ++i) {
      ...
      } else if (MageStudios.ElementIdRef[ele]) {
        array.push(MageStudios.ElementIdRef[ele]);
      }
    }
...
Game_Action.prototype.getItemElements = function() {
  ...
  var elements = [];
  if (this.item().daMageStudios.elementId < 0) {
    MageStudios.Util.extend(elements, this.subject().attackElements());
  } else if (this.item().daMageStudios.elementId > 0) {
    elements.push(this.item().daMageStudios.elementId);
  }
  MageStudios.Util.extend(elements, this.item().multipleElements);
  if ($gameTemp._addedElements !== undefined) {
    MageStudios.Util.extend(elements, $gameTemp._addedElements);
  }
