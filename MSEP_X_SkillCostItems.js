 ...
 *   before it can be used. If you are using MSEP_ItemCore, independent items
 ...
 *   weapons, then armors. If you are using MSEP_ItemCore, independent items
 *   cannot be used as item costs as the game will not distinguish which of the
 *   independent items is fit to be used. If you use multiple of these tags,
 *   the skill will require all the listed items to be available for usage.
 *
...
if (Imported.MSEP_SkillCore) {
  ...
  Window_SkillList.prototype.drawSkillItemCost = function (skill, wx, wy, dw) {
    ...
    var max = array.length;
    if (max <= 0) return dw;
    this.contents.fontSize = MageStudios.Param.SCIFontSize;
    dw -= 2;
    for (var i = 0; i < max; ++i) {
  ...
  Window_SkillList.prototype.drawSoloItemCost = function (
    ...
    var amt2 = MageStudios.Util.toGroup(this._actor.numItems(item));
    var fmt = MageStudios.Param.SCIAmountFmt;
