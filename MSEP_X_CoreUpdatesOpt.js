...
MageStudios.Updates.version = 1.0;
...
if (MageStudios.PluginRequirements()) {
  ...
  MageStudios.SetupParameters = function () {
    ...
    MageStudios.Param.UpdateDesktop = eval(
      String(MageStudios.Parameters["Desktop_Updates"])
    );
  };
  ...
  if (MageStudios.Param.Update110) {
    ...
    console.log(
      "RPG Maker Version " + Utils.RPGMAKER_VERSION + " updates loaded"
    );
  ...
  if (MageStudios.Param.Update130) {
    ...
    Bitmap.prototype.bltImage = function (
      source,
      sx,
      sy,
      sw,
    ...
    TilingSprite.prototype._renderCanvas_PIXI =
      PIXI.extras.TilingSprite.prototype._renderCanvas;
    TilingSprite.prototype._renderWebGL_PIXI =
      PIXI.extras.TilingSprite.prototype._renderWebGL;
    ...
    ToneFilter.prototype = Object.create(
  ...
  if (MageStudios.Param.Update131) {
    ...
    ScreenSprite.warnYep = function () {
      if (!ScreenSprite.YEPWarned) {
        console.log(
          "Deprecation warning. Please update MSEP_CoreEngine. ScreenSprite is not a sprite, it has graphics inside."
        );
  ...
  if (MageStudios.Param.Update150) {
    ...
    Bitmap.prototype._renewCanvas = function () {
      ...
      if (
        newImage &&
        this.__canvas &&
        (this.__canvas.width < newIMageStudios.width ||
          this.__canvas.height < newIMageStudios.height)
      ) {
        this._createCanvas();
      }
    };
        ...
        case "decrypting":
          ...
          if (!this._loader) {
            this._loader = ResourceHandler.createLoader(
              this._url,
              this._requestIMageStudios.bind(this, this._url),
              this._onError.bind(this)
            );
            this._iMageStudios.removeEventListener(
              "error",
              this._errorListener
            );
            this._iMageStudios.addEventListener(
              "error",
    ...
    Bitmap.prototype._requestImage = function (url) {
      ...
      if (this._decodeAfterRequest && !this._loader) {
        this._loader = ResourceHandler.createLoader(
          url,
          this._requestIMageStudios.bind(this, url),
          this._onError.bind(this)
        );
      }
    ...
    Decrypter.decryptImg = function (url, bitmap) {
      ...
      requestFile.onload = function () {
        if (this.status < Decrypter._xhrOk) {
          var arrayBuffer = Decrypter.decryptArrayBuffer(requestFile.response);
          bitmap._iMageStudios.src = Decrypter.createBlobUrl(arrayBuffer);
          bitmap._iMageStudios.addEventListener(
            "load",
  ...
  var text = "";
  text +=
    "You are getting this error because you are trying to run MSEP_X_CoreUp";
  text +=
    "datesOpt while your project files are lower than version 1.4.0. \n\n";
