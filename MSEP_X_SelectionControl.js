 ...
 * @on YES
 * @off NO
 * @desc Req. MSEP_RowFormation, set physical single-target skills
 ...
 * @on YES
 * @off NO
 * @desc Req. MSEP_RowFormation & if the above is false, set physical
 ...
 * @on Ranged
 * @off Melee
 * @desc Req. MSEP_RowFormation, the default range for weapons.
 ...
 * ============================================================================
 *
 * This plugin requires MSEP_BattleEngineCore and MSEP_TargetCore. Make sure this
 ...
 *
 * If you are using the MSEP_X_BattleSysCTB plugin, place this plugin under that
 ...
 *   <Weapon Range>
 *   <Weapon Ranged>
 *   Requires MSEP_RowFormation.js. This will make the selection of targets vary
 ...
 *
 * Any Row
 * - Requires MSEP_RowFormation.js. The battler can be targeted from any row it
 ...
 *
 * Back Row Only
 * - Requires MSEP_RowFormation.js. This will make only the back row battlers
 ...
 *
 * Front Row Only
 * - Requires MSEP_RowFormation.js. This will make only the front row battlers
 ...
 *
 * Row x Only
 * - Requires MSEP_RowFormation.js. This will make only battlers in row x be
 ...
 *
 * Row x Max
 * - Requires MSEP_RowFormation.js. This will make all battlers who are located
 ...
 *
 * Version 1.09:
 * - Fixed an exploit with skills that gain TP across Disperse DaMageStudios.
 *
 * Version 1.08:
...
Game_Battler.prototype.checkActionSelectable = function(line, action) {
    ...
    if (line.match(/ITEM[ ](.*)/i)) {
      if (action.isItem()) {
        ...
        } else if (MageStudios.ItemIdRef[text]) {
          var id = MageStudios.ItemIdRef[text];
          return action.item().id !== id;
        } else {
          return true;
        }
    ...
    if (line.match(/SKILL[ ](.*)/i)) {
      if (action.isSkill()) {
        ...
        if (text.match(/(\d+)/i)) {
          var id = parseInt(RegExp.$1);
          return action.item().id !== id;
        } else if (MageStudios.SkillIdRef[text]) {
          var id = MageStudios.SkillIdRef[text];
          return action.item().id !== id;
        } else {
          return true;
        }
    ...
    if (line.match(/STYPE[ ](.*)/i)) {
      if (action.isSkill()) {
        ...
        } else if (MageStudios.STypeIdRef[text]) {
          var id = MageStudios.STypeIdRef[text];
          return action.item().stypeId !== id;
        } else {
          return true;
        }
    ...
    if (line.match(/ELEMENT[ ](.*)/i)) {
      ...
      if (text.match(/(\d+)/i)) {
        var id = parseInt(RegExp.$1);
        return action.item().daMageStudios.elementId !== id;
      } else if (MageStudios.STypeIdRef[text]) {
        var id = MageStudios.STypeIdRef[text];
        return action.item().daMageStudios.elementId !== id;
      } else {
...
Game_Action.prototype.selectConditionBackRow = function(target) {
    ...
    for (var i = MageStudios.Param.RowMaximum; i > 0; --i) {
...
Game_Action.prototype.selectConditionNotState = function(target, text) {
    if (text.match(/(\d+)/i)) {
      ...
      var id = MageStudios.StateIdRef[text];
    } else {
      return false;
    }
